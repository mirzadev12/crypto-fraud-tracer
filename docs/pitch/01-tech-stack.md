...
| Attribution data | 15 tagged exchange wallets + 241 derived deposit addresses + 334 OFAC-listed TRON addresses | 590 labels in one in-memory map at startup |
| Other chains | 709 more OFAC-listed addresses on 19 other assets, in `data/sanctions-multichain.json` | An address from any chain OFAC covers is recognised by format and screened; only TRON is traced |
...
| Offline tooling | 12 Node ESM scripts in `scripts/` | Clustering, case capture, re-derivation, calibration, verification, sanctions refresh |
...
```
  ...
  Next.js application
    route handlers  →  tracer  →  chain client (cache · pacing · SHA-256)
                  ...
                  15 seeds · 241 deposit addresses · 334 sanctioned
        │
        ▼
...
**Seven endpoints:** trace (POST), trace permalink (GET, replays one run exactly),
transaction lookup, wallet profile, watch check, sanctions screening for any
chain, health.
...
| Sanctioned TRON addresses carried | **334** | OFAC SDN list published 18 Sep 2026 — every entry that decodes as a TRON address (254 filed under TRX, 79 under USDT, 1 under XBT) |
| Total attribution labels | **590** | 15 + 241 + 334, no overlaps |
| Addresses screened on any chain | **1,043** across **20** assets | the same list; screening, not tracing |
| Real cases frozen with response hashes | **10** | `data/demo-cases.json` |
| Behavioural rules | **6** | `lib/risk.ts` |
| API endpoints | **7** | `app/api/**` |
